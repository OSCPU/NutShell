...
class ReqBlocker(n: Int, userBits:Int = 0) extends Module {
  ...
  switch (state) {
    ...
    is (s_hi)          { when (io.in(1).req.fire) { state := s_waitResp_hi } }
    is (s_lo)          { when (io.in(0).req.fire) { state := s_waitResp_lo } }
    is (s_waitResp_hi) { when (io.in(1).resp.fire) { state := s_idle } }
    is (s_waitResp_lo) { when (io.in(0).resp.fire) { state := s_idle } }
  }
