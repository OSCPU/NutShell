...
class WakeupBus extends NutCoreBundle with HasBackendConst {
  val pdest = Output(UInt(prfAddrWidth.W)) // generated by RS
  val data = Output(UInt(XLEN.W)) // result from FU
  // cycle 0: RS select an inst ready for issue, 
  //          send pdest to WakeupBus, 
  //          set WakeupBus.valid to true
  // cycle 1: FUs write data to WakeupBus, RS receives data from WakeupBus
}