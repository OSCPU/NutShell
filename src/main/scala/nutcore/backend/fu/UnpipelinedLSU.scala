...
class LSExecUnit extends NutCoreModule {
  ...
  io.out.valid := Mux( dtlbPF && dtlbEnable || io.loadAddrMisaligned || io.storeAddrMisaligned, valid, Mux(partialLoad, state === s_partialLoad, dmem.resp.fire() && (state === s_wait_resp)))
